...
def fed_norms(model, aggr_result):
    
    # global_model.load_state_dict(avg_state_dict)
    device = torch.device("cuda:0")  # 将全局模型和局部模型都移动到 cpu 设备上
    list2, local_state_dicts = list(zip(*aggr_result))
    ...
    for layer in avg_state_dict.keys():
        ...
        for client_idx in range(len(aggr_result)):
            local_state_dict = local_state_dicts[client_idx]
            for param_name, param in local_state_dict.items():
                local_state_dict[param_name] =  param.to(device)  # 将局部模型的权重移动到 cpu 设备上
            
            avg_state_dict[layer] = avg_state_dict[layer].float()
            avg_state_dict[layer] += local_state_dict[layer] # 
        avg_state_dict[layer] = avg_state_dict[layer].float()
        avg_state_dict[layer] /= len(aggr_result)
...
if __name__ == '__main__':
    ...
    if params_loaded['type'] == config.TYPE_LOAN:
        ...
        helper.load_data()
    elif params_loaded['type'] == config.TYPE_SUBIMAGENET:
        helper = ImageHelper(current_time=current_time, params=params_loaded,
                             name=params_loaded.get('name', 'sub'))
        helper.load_data()
    elif params_loaded['type'] == config.TYPE_WEBFACE:
        helper = ImageHelper(current_time=current_time, params=params_loaded,
                             name=params_loaded.get('name', 'webface'))
        helper.load_data()
    else:
        helper = None
    ...
    for epoch in range(helper.start_epoch, helper.params['epochs'] + 1, helper.params['aggr_epoch_interval']):
        ...
        epochs_submit_update_dict, num_samples_dict , merged_res= train.train(helper=helper, start_epoch=epoch,
        ...
        if helper.params['aggregation_methods'] == config.AGGR_MEAN:
            # Average the models
            # helper.average_models_params(epochs_submit_update_dict,agent_name_keys,target_model=helper.target_model)
            ...
            #debug
            # t_model = copy.deepcopy(helper.target_model)
            # global_model_dict  = t_model.state_dict()
            # config_norm = add_args()
            # config_norm.norm_bound = 0.1  # for norm diff clipping; in the paper, they set it to 0.1, 0.17, and 0.33.
            # defense = NormDiffClippingDefense(config_norm)
            # aggr_result = defense.defend_before_aggregation(merged_res, global_model_dict)
            # helper.target_model = fed_norms(helper.target_model, aggr_result)
